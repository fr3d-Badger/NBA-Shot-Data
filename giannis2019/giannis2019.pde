...
color playerplot = color(251, 128, 0);
color teambanner = color(0,71,27);

void setup() {
  beginRecord(PDF, "giannis2019.pdf");
  //size of court in feet x10
  size(500, 470);
  background(teambackground);
  
  //Kevin Durant MVP year
  table = loadTable("NBA_2019_Shots.csv", "header");
}
...
void plotShots(){
  ...
  for (TableRow row : table.rows()) {
    ...
    if (name.equals("Giannis Antetokounmpo") && make.equals("TRUE")) {
