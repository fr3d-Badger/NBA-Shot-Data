...
color teambackground = color(6, 25, 34);
color playerplot = color(6, 25, 34);

void setup() {
  beginRecord(PDF, "harden2018.pdf");
  //size of court in feet x10
  size(500, 470);
  background(0);
  
  //Kevin Durant MVP year
  table = loadTable("NBA_2018_Shots.csv", "header");
}
...
void plotShots(){
  ...
  for (TableRow row : table.rows()) {
    ...
    if (name.equals("James Harden") && make.equals("TRUE")) {
